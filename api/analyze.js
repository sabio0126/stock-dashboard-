...
各要因について、必ずWeb検索を行い、その根拠となった具体的なニュース記事を特定してください。対象とするニュースは「本日から3日以内に発行された新しい記事」に限定してください。3日より古い記事は使わないでください。そして、その記事の「発行日時」を "datetime"（できるだけ "YYYY-MM-DD HH:MM" の形式で、時刻まで。時刻が不明なら "YYYY-MM-DD" のみ）、媒体名を "source"（例："日経新聞"、"Reuters"）、記事のURLを "url" として必ず付けてください。Web検索をして実際の記事を見つけてから要因を書いてください。日時が本当に特定できない場合のみ "datetime" を空文字 "" にしてください。URLも特定できない場合のみ空文字 "" にしてください。直近3日以内に該当するニュースがない場合は、その要因の配列は空にしてください。
...
さらに、各銘柄について「今後1か月以内に予定されている、株価に影響を及ぼしそうなイベント」も調べてください。例：決算発表、新製品発表、株主総会、製品ローンチ、規制当局の判断期日、重要な業界カンファレンスなど。1か月より先のイベントは含めないでください。予定が見つからない場合は空配列にしてください。
  ...
  "analysis": [
    {
      ...
      "bullish": [
        { "text": "上昇要因の内容", "impact": "強 または 中 または 弱", "datetime": "YYYY-MM-DD HH:MM", "source": "媒体名", "url": "記事URL" }
      ],
      "bearish": [
        { "text": "下落要因の内容", "impact": "強 または 中 または 弱", "datetime": "YYYY-MM-DD HH:MM", "source": "媒体名", "url": "記事URL" }
      ],
      "events": [
